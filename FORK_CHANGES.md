...
- UI smoke tests use a layered harness under `Go CyclingUITests/`: `Support/` (`Timeouts`, `ElementAssertions`, `BaseTestCase`, `CycleTestCase`, `StatisticsTestCase`), `Screens/` (queries and actions), `Flows/` (`ResetAppDataFlow` with selectable reset areas), `Assertions/` (`Screen+Assertions.swift` composite expectations), and `Tests/` including `HistorySmokeTests` and `StatisticsSmokeTests`.
